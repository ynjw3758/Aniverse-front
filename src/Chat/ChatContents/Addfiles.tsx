...
const Addfiles =(props:info) =>{
    ...
    useEffect(() =>{
        console.log("props_img :" , props.Img);
        console.log("props_video :" , props.Video);
        if(props.Img.length !=0 && props.Video.length == 0){
            console.log("이미지");
            console.log("isimg :" , isimg , " isvideo :" , isvideo);
            if(isimg == true && isvideo== true){
                setIsvideo(false);
                return;
            }
            else if(isimg == false){
                console.log("img :" , props.Img , ", video :" , props.Video);
                setIsimg(true);
                return;
            }
    ...
    return(<> 
        {(isimg == true && isvideo ==false) && (<div className="AddChatFile_total">
          {props.ImgId.map((data, i) =>(<>
             <AddFileList Img={props.Img[i]} Video={""} imgid={data} videoid={""}/>
           </>))}
         </div>)}
         {(isvideo == true && isimg == false) && (<div className="AddChatFile_total">
            {props.Video.map((data, i) =>(<>
             <AddFileList Img={""} Video={data} imgid={""} videoid={data}/>
             </> ))}
          </div>)}
